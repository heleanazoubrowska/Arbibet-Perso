                    ...
                    try
                    {
                        ...
                        try
                        {
                            ...
                            try
                            {
                                ...
                                //getOdds(Ph2888, htmlDoc);
                            }

                            catch
                            {
                                NLogger.Log(EventLevel.Error, "Ph2888 odds table not found : 003 ");
                            }

                        }
                        catch (NoSuchElementException elemNotFound)
                        {
                            NLogger.Log(EventLevel.Error, "Ph2888 odds table not found : 002 " + elemNotFound);
                        }
                    }
                    catch
                    {
                        NLogger.Log(EventLevel.Error, "Ph2888 probably in maintenance");
                    }
                }
                catch (NoSuchElementException elemNotFound)
                {
                    NLogger.Log(EventLevel.Error, "Ph2888 login not found : 001 " + elemNotFound);
                }
            }
            catch
            {
                NLogger.Log(EventLevel.Error, "Ph2888 : Fail to connect ");
            }
        }
